...
# Echo se utiliza para imprimir en la consola
echo "Hola mundo"

# A diferencia de otros lenguajes se declara sin tener que poner el tipo de dato qué es y sin agregar el espacio antes ni despues del signo de igual.
variable="Eddy Leonardo"

clear # limpiar la pantalla

# Tambien agregar el simbolo de moneda "$" antes de la variable.
# Cabe destacar que para declarar una variable se necesita poner doble comillas, 

echo "$variable" # => Eddy Leonardo
echo '$variable' # => $variable

# Los array se declaran colocando entre paréntesis los valores y sin usar comillas.
array=(Pedro, María, José, Pedro, Juan)

# Para obtener la cantidad de carácteres hay que utilizar:
echo "${#variable}" # => 15
...
# puedes utilziar esta forma de imprimir todos los valores en un rango con llaves
...
read age
if [[ "$name" == "Eddy" ]] && [[ "$age" -eq 16 ]]; then
    echo "This will run if $name is Eddy AND $age is 16."
fi

if [[ "$name" == "Sergio" ]] || [[ "$name" == "Manolo" ]]; then
    echo "This will run if $name is Sergio OR Manolo ."
fi
